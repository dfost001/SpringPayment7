...
var processAddress = function(){
    ...
    var fillRequest = function(){
        ...
        console.log("addressVerify#fillRequest: city=" + request.city);
        request.state = $("#state").val();
        request.zipcode = $("#postalCode").val();
        request.country = $("#countryId").val();
    ...
    */
    ...
    var processAddressVerifySuccess = function(payload) {
        ...
        debugPrint();
        assignReformattedFields();
        processErrors();
        processMessages();     
        
        if(response.confirmRequired)
            $("#btnConfirm").show(500);
        else if(response.valid) //if there are no other changes, just assign the full zip
    ...
    var confirmHandler = function(event){
        event.preventDefault();
            ...
            if(response.deliveryLine !== null && response.deliveryLine !== "")  
                   $("#address1").val(response.deliveryLine);
            ...
            if(response.zipPlus4 !== null && response.zipPlus4 !== "")            
                $("#postalCode").val(response.zipPlus4); 
            else if(response.zip !== null && response.zip !== "")
                $("#postalCode").val(response.zip);
           
           $("#btnConfirm").hide(500); 
           
           // onChangeShowAlert(); --revision will not change validity so no need to re-verify
    };
   ...
   var attachVerify = function() {
    ...
    //Invoked onload and from key press handlers on customerForm.js
     var onChangeShowAlert = function() {
        ...
        var html = "Please re-verify after edits are complete. ";
